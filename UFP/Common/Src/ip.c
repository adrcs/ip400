...
void Get172AddrFromID(IP400_MAC *fr, SOCKADDR_IN *ipaddr)
{
	...
	uint8_t b3 = (fr->callbytes.callsign.bytes[0] ^ fr->callbytes.callsign.bytes[2]) & netmask172[2];
	uint8_t b4 = (fr->callbytes.callsign.bytes[1] ^ fr->callbytes.callsign.bytes[3]) & netmask172[3];
