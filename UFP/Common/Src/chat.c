...
char *welcome = "Welcome to chat. \r\nESC to set destination\r\nCTRL/E to enter echo mode\r\nCTRL/Z to exit\r\n";
...
{
	IP400_FRAME *SrcFrame = (IP400_FRAME *)raw;

	if(SrcFrame->flagfld.flags.coding == ECHO_RESPONSE)	{
		PrintFrame(SrcFrame);
		DeleteFrame(SrcFrame);
		return TRUE;
	}
	...
	if(!enqueFrame(&chatQueue, SrcFrame, SrcFrame->length))
		return FALSE;
...
BOOL Chat_Task_exec(void)
{
	...
	if((fr=dequeFrame(&chatQueue)) != NULL)	{
		PrintFrame(fr);
		if(fr->hopTable != NULL)
			nodeMemFree(CHAT, fr->hopTable);
		if(fr->buf)
			nodeMemFree(CHAT, fr->buf);
		nodeMemFree(CHAT, fr);
	}
...
void sendLine(char *buffer, int len)
{
		if(!SendTextFrame(setup_memory.params.setup_data.stnCall, GetVPNLowerWord(), dest_call, dest_ip, buffer, len, repeat))
			USART_Print_string("Not sent..\r\n");
}

void sendEchoreq(void)
{
	char *echoPayload = "Echo request frame";
	int len = strlen(echoPayload);

	SendEchoReqFrame(setup_memory.params.setup_data.stnCall, GetVPNLowerWord(), dest_call, dest_ip, echoPayload, len, FALSE);
}
...
void ListAllMeshEntries(char *call, int len, int nMACEntries)
{
	...
	if((MacEntries = (struct ip400MAC_t *) nodeMemAlloc(CHAT,nMACEntries * sizeof(struct ip400MAC_t))) == NULL)		{
...
void PrintFrame(IP400_FRAME *FrameBytes)
{
	...
	// source call
	callDecode(FrameBytes, decCall, NULL, SRC_CALLSIGN);
	...
	// dest call
	if((FrameBytes->dest.callbytes.callsign.bytes[0] == BROADCAST_ADDR)	 &&
		(FrameBytes->dest.callbytes.callsign.bytes[1] == BROADCAST_ADDR)) {
		USART_Print_string("BROADCAST");
	} else {
		callDecode(FrameBytes, decCall, NULL, DEST_CALLSIGN);
	...
	// flags
	USART_Print_string("[%04d]:", FrameBytes->seqNum);
