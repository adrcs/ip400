					      ...
					      time_t timeout)
{
	...
	if (s_err == STM32_ERR_NACK)
		...
		fprintf(stderr, "Unexpected reply (%02x)from device on command 0x%02x\n", s_err, cmd);
	return STM32_ERR_UNKNOWN;
}
...
stm32_t *stm32_init(struct port_interface *port, const char init)
{
	...
	// stm32WL return a langth then the option bytes
	if(is32WL())	{
	    	int l = buf[0];
		if(l != 2)	{
			fprintf(stderr, "STM32WL device length incorrect %d\n", l);
		} else {
			stm->pid = (buf[2] << 8) | buf[3];
		}
	} else {
		if (len > 2) {
			fprintf(stderr, "This bootloader returns %d bytes in PID:", len);
			for (i = 0; i < len ; i++)
